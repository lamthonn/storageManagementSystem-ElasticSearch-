...
namespace DATN.Application.DanhMucPhongBan
{
    public class DanhMucPhongBanService : IDanhMucPhongBan
    {
        private readonly AppDbContext _context;
        public DanhMucPhongBanService(AppDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<danh_muc_dto> Create(danh_muc_dto obj)
        {
            try
            {
                var exitDanhMuc = _context.danh_muc.Any(x => x.ma == obj.ma && x.ma_dinh_danh == "danh-muc-phong-ban");
                if (exitDanhMuc) throw new Exception("Mã danh mục đã tồn tại!");
                var entity = new danh_muc
                {
                    Id = Guid.NewGuid(),
                    ma_dinh_danh = "danh-muc-phong-ban",
                    ma = obj.ma,
                    ten = obj.ten,
                    mo_ta = obj.mo_ta,
